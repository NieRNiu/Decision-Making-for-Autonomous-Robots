...
private:
    ...
    std::string srv_assert_knowledge_name_; /// name of the servic eprovided by reasoning node
    ...
    std::vector<std::string> v_seen_obj_resoning_;  ///< List of objects seen by the robot and sent to the reasoning

    ros::ServiceClient client_map_generator_; ///< Client to request the object list update in the map generator node
    ros::ServiceClient client_reasoning_; ///< Client to request the object list update in the reasoning node
  ...
  void topic_callback(const gazebo_msgs::ModelStates::ConstPtr& msg){
   ...
   auto it = std::find( msg->name.begin(),  msg->name.end(), "tiago");
    ...
    for (int i = 0; i < msg->name.size(); i++)
    {
        ...
        //COPY: obtain the dx distance between the robot and the objects (0.5 pts)
        double dx= tiago_pose.position.x - obj_pose.position.x;
        //COPY: obtain the dy distance between the robot and the objects (0.5 pts)
        double dy= tiago_pose.position.y - obj_pose.position.y;
        //COPY: compute the distance between the tiago and the objects (1pt)
        double d= sqrt(dx*dx + dy*dy);

        //IF the robot is closer to the seen objects, then request the service
        if (d<20)
        {
            //COPY: Identify the object seen (0.5 pts)
            ...
            if (it == v_seen_obj_.end()) {
                ...
                //COPY: send the new seen object to the service (0.25 pts)
                srv.request.object_name=s;
                //COPY: send the pose of the seen object to the service (0.25 pts)
                srv.request.object_pose= obj_pose;

                //call map generator node
            ...
            if (it2 == v_seen_obj_resoning_.end()) {
                ...
                //call reasoning node
                world_percept_assig3::UpdateObjectList srv2;
                //COPY: send the new seen object to the service (0.25 pts)
